...
#define CAN_ID_USINE 0x034  // ID 
#define CAN_ID_VOITURE 0x032
#define ORANGE 'O'
#define METAL 'M'
...
volatile int stop_non_can_threads = 0;
...
void *thread_can_attente(void *arg) {
    ...
    while (1) {
	    ...
	    if (nbytes > 0) {
		    ...
		    // Si l'ID du message est celui de l'usine
		    if (frame.can_id == CAN_ID_USINE) {
                
			    printf("Thread CAN: Message reçu. Lancement des threads...\n");
			    
			    if(frame.data[1]==0x63 || frame.data[1] == 0x62){
				    
				    if(frame.data[1] == 0x62){BlockColor = ORANGE;}
				    else{BlockColor = METAL;}
			    }
		   ...
		   if(frame.can_id == CAN_ID_USINE){
			   if(frame.data[2] == 0x81){ flag = 1;}}
		if(frame.can_id == CAN_ID_VOITURE){
		   	
			   if(frame.data[0] == 0x36 && flag == 1){
            		flag = 0;
...
// Attente du signal de démarrage
   ...
   // while (!start_threads) {
     //   pthread_cond_wait(&cond_start, &mutex_start);
    //}
    while (system_state != RUNNING && !stop_threads && !stop_non_can_threads) {
        pthread_cond_wait(&cond_start, &mutex_start); // Attente de passage à "RUNNING"
    }
    pthread_mutex_unlock(&mutex_start);
	
    while (!stop_threads && !stop_non_can_threads) {
        float distance;
